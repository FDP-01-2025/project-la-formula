   ...
   int showMainMenu()
{
    ...
    PlaySoundA("MenuSong.wav", NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);

    showLogo();                                   // Mostrar logo animado
    rlutil::msleep(3000);                         // Esperar 3 segundos
    rlutil::cls();                                // Limpiar pantalla
    createMenuArtFile();                          // Crear arte del menú
    int selection = interactiveMenu("arte2.txt"); // Mostrar menú interactivo
    remove("arte2.txt");

    //Cambiar música según la opción elegida
    switch (selection)
    {
    case 0: // New Game
    ...
    case 1: // Continue Game
        ...
        break;
    case 2: // Exit
        PlaySoundA(NULL, NULL, 0); // Detener música
        break;
    }
...
{
    PlaySoundA(NULL, NULL, 0);  // Detiene la música
}
